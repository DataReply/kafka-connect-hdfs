...
public class AvroData {
    ...
    public org.apache.avro.Schema fromConnectSchema(Schema schema) {
        ...
        if (schema.name() != null) {
            ...
            if (schema.name().contains("Decimal")) {
                baseSchema.addProp("logicalType", "decimal");
                baseSchema.addProp("precision", JsonNodeFactory.instance.numberNode(20));
            }
        }
...
//      baseSchema.addProp(CONNECT_PARAMETERS_PROP, parametersFromConnect(schema.parameters()));
            for (Map.Entry<String, String> entry : schema.parameters().entrySet()) {
                try {
                    ...
                    baseSchema.addProp(entry.getKey(), JsonNodeFactory.instance.numberNode(value));
                } catch (Exception e) {
